...
import { getPatientData } from './services/patientService'; // [NEW]
import {
  calculateAge,
  calculateBMI,
  calculateWHR,
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const somatotype = calculateSomatotype(consultation, patient.height, consultation.weight);
  ...
  const handleGenerateSummary = async () => {
    ...
    try {
      ...
      setDiagnosticSummary("Error al generar el resumen. Por favor intente de nuevo.");
    } finally {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const response = await saveConsultation(patient, consultation, results, diagnosticSummary);
      if (response.success) {
    ...
    <div className="flex h-screen bg-slate-50 font-sans text-[#003844]">
      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8">
          <PatientCard patient={patient} age={results.age} genderLabel={patient.gender === 'F' ? 'Femenino' : 'Masculino'} />

          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            {/* Left Column: Data Entry */}
            <div className="lg:col-span-7 space-y-8">
              <PatientForm patient={patient} onChange={handlePatientChange} />
              <ConsultationForm consultation={consultation} results={results} onChange={handleConsultationChange} />
            </div>
